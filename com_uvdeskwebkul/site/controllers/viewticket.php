...
class UvdeskwebkulControllerViewticket extends JControllerForm
{
	...
	function postReply(){
		...
		$ch = curl_init($url);
		$json_id=array("threadType"=>"reply", "reply"=>$postData['content'],"status"=>$postData['replyStatus'],"actAsType"=>"customer","actAsEmail"=>JFactory::getUser()->email);
		$headers = array('Authorization: Bearer '.$access_token,);
		...
		$response = substr($output, $header_size);   
		$response=json_decode($response); 
		$this->setRedirect('index.php?option=com_uvdeskwebkul&view=viewticket&id='.$ticketId,$response->message);
	}
}
